...
---
## Some Considerations
Note that we do not have to pass the data frame's name as an argument to functions within a pipe.

--

Using `%>%` makes a copy of the original data frame. You have to save your manipulated data frame to use it again later.
...
--
It is often necessary to manipulate your data frame in a specific way to generate the plot you envision. Saving copies prevents confusion and therefore bugs.
...
Logical statements should be enclosed within parentheses and joined with `&` (AND) or `|` (OR)
...
```{r eval = FALSE, echo = TRUE}
flights %>%
  filter(((origin=="SFO") & (dest %in% c("BOS","PHL"))))
```
